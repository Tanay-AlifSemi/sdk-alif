...
//#define BUFF_SIZE  175

/* Master and Slave buffer word size */
#define SPI_WORD_SIZE 8

/* Master and Slave buffer frequency */
//#define SPI_FREQUENCY (1 * Mhz) //working
//#define SPI_FREQUENCY (10 * Mhz) //working
//#define SPI_FREQUENCY (16 * Mhz) //(1 * Mhz) //working
//#define SPI_FREQUENCY (20 * Mhz) //(1 * Mhz) //working
//#define SPI_FREQUENCY (24 * Mhz) //(1 * Mhz)
//#define SPI_FREQUENCY (30 * Mhz) //(1 * Mhz) //not possible due to baud divider lsb always 0-> o/p 40 MHz
//#define SPI_FREQUENCY (40 * Mhz) //(1 * Mhz) //not working
//#define SPI_FREQUENCY (80 * Mhz) //(1 * Mhz) //lpspi master can send at 160/2 = 80 MHz //turbo -> actual 120MHz , div 3 -> lsb 0 -> div -> 2

//#define SPI_FREQUENCY (120 * Mhz) //(1 * Mhz) //normal> 160 will not generatle clock as div->1 // lpspi master can send at 160/2 = 80 MHz //turbo -> actual 120MHz , div 3 -> lsb 0 -> div -> 2

/* LPSPI slave full-duplex (TRM): SPI_CLK >= 12 * SCK.
 * Turbo LPSPI SSI=240 MHz => SCK max 20 MHz. 60 MHz is only 4x and
 * hangs DMA (debugger cannot halt) at BUFF_SIZE=200 (800 bytes).
 */
#define SPI_FREQUENCY (20 * Mhz)
//#define SPI_FREQUENCY (60 * Mhz) /* illegal for LPSPI slave TX+RX; hangs @ 800B */
//#define SPI_FREQUENCY (40 * Mhz)
//#define SPI_FREQUENCY (30 * Mhz)

/* Master and Slave buffer transfers */
//#define SPI_NUM_TRANSFERS  10
#define SPI_NUM_TRANSFERS  1000
...
static void master_spi(void *p1, void *p2, void *p3)
{
	...
	while (iterations) {
		...
		k_msleep(SLEEPTIME);
		if (ret < 0) {
			printk("Stopping the Master Thread due to error %d\n", ret);
			return;
		}
		iterations--;
	}
...
static void slave_spi(void *p1, void *p2, void *p3)
{
	...
	while (iterations) {
		...
		if (ret < 0) {
			printk("Stopping the Slave Thread due to error %d\n", ret);
			return;
		}
		iterations--;
	}
