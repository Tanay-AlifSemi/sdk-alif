...
/*
 * Pairing: SPI1 master (HCLK 80/120) + LPSPI slave (pll_clk1 160/240).
 *
 * 10 MHz is an exact SCK on both master SSI clocks:
 *   80/8 = 10 MHz,  120/12 = 10 MHz  (BAUDR even).
 * 8 MHz is NOT fair (80→8.00, 120→8.57). 15/16/22/24/30 are not either.
 *
 * LPSPI slave TX+RX TRM: SSI >= 12*SCK → max 13.3 MHz (160) / 20 MHz (240).
 */
...
/* Same actual SCK at HCLK 80 MHz and 120 MHz. */
...
static uint32_t g_ssi_hz;	/* master SSI (SPI1 HCLK) */
static uint32_t g_slave_ssi_hz;	/* LPSPI SSI */
...
static uint32_t read_node_ssi(const struct device *clkdev,
...
static uint32_t read_master_ssi(void)
{
	return read_node_ssi(DEVICE_DT_GET(DT_CLOCKS_CTLR(SPIDW_NODE)),
			     (clock_control_subsys_t)DT_CLOCKS_CELL(SPIDW_NODE, clkid),
			     DT_PROP(DT_NODELABEL(syst_hclk), clock_frequency));
}

static uint32_t read_slave_ssi(void)
{
	return read_node_ssi(DEVICE_DT_GET(DT_CLOCKS_CTLR(S_SPIDW_NODE)),
			     (clock_control_subsys_t)DT_CLOCKS_CELL(S_SPIDW_NODE, clkid),
			     DT_PROP(DT_PATH(clocks, pll_clk1), clock_frequency));
}
...
static int slave_spi_transceive(const struct device *dev)
{
	...
	if (ret < 0) {
		/* After the last xfer the slave rearms; main never clocks again.
		 * Driver slave DMA wait is 10 s → -ETIMEDOUT. Not a test fail.
		 */
		if (g_slave_run || ret != -ETIMEDOUT) {
			...
			uint32_t freq, uint32_t *cycles_out)
{
	...
	if (k_sem_take(&sem_slave_done, K_SECONDS(2))) {
...
static void print_mode_banner(void)
{
	...
	printk("SPI mode: normal (SYS_CLOCK_HW_CYCLES_PER_SEC=%u)\n",
	       CONFIG_SYS_CLOCK_HW_CYCLES_PER_SEC);
#endif
	printk("Pairing: SPI1 master + LPSPI slave (full duplex)\n");
	printk("Master SSI (HCLK get_rate)=%u  BAUDR(10MHz)=%u  actual SCK=%u\n",
	       g_ssi_hz, baudr_of(g_ssi_hz, SPI_BENCH_FREQ),
	       actual_sclk(g_ssi_hz, SPI_BENCH_FREQ));
	printk("Slave SSI (LPSPI get_rate)=%u  TRM 12x max SCK=%u\n",
	       g_slave_ssi_hz,
	       g_slave_ssi_hz ? (g_slave_ssi_hz / 12U) : 0U);
}
...
int main(void)
{
	...
	g_ssi_hz = read_master_ssi();
	g_slave_ssi_hz = read_slave_ssi();
	print_mode_banner();
